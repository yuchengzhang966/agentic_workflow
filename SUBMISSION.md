...
- **数据持久化** —— 目前用 LangGraph `MemorySaver`(内存版),进程重启后流水线状态丢失。需换成 `SqliteSaver`,改动很小、已定位。**这是相对作业要求的主要缺口。**
...
1. **`MemorySaver` → `SqliteSaver`** —— 满足"数据持久化"硬性要求,流水线状态可跨重启恢复。
2. **服务器升级到 Python 3.10+ venv 并启用 e2b** —— 生成的应用走真实 HTTPS 隔离沙箱,而非本地兜底。

### P1 — 从"能跑"到"可用、可信"

3. **项目列表 + 历史** —— 每次生成入库,可回看、可继续。
4. **错误恢复** —— agent 失败时的重试与友好报错(目前部分错误偏静默)。

### P2 — 产品纵深

5. **多轮迭代** —— 对已生成的应用继续提需求、改代码,形成真正的 vibe coding 闭环。
6. **注册 / 登录** —— 生成产物归属到用户。
7. **重新接上 Reviewer 自动修复循环** —— 用质量分驱动 Engineer 迭代。

### 优先级判断依据

P0 是**达标线** —— 作业明确要求"数据持久化"与"实际可用",必须先满足;
